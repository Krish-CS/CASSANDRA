...
def generate_preview():
    ...
    try:
        ...
        try:
            ...
            num_slides = 15
        content_mode = data.get('content_mode') or 'cassandra'
        ...
        try:
            ...
            if user_titles and isinstance(user_titles, list) and len(user_titles) > 0:
                ...
                slide_titles = (topics_result or {}).get("slides") or _get_default_slides(topic)
...
def refine_slide():
    ...
    try:
        ...
        style = data.get('style') or 'bullet'  # 'paragraph' or 'bullet'
        bullet_symbol = data.get('bullet_symbol') or '➣'  # Use selected bullet style
        ...
        try:
            ...
            if style == "bullet":
                lines = str(new_content).strip().split('\n')
                formatted_lines = []
                for line in lines:
                    line = line.strip()
                    if not line:
...
def generate_ppt():
    ...
    try:
        ...
        template_style = data.get('template') or 'modern'
        template_url = data.get('templateUrl') or ''  # Pexels background URL
        mode = data.get('mode') or 'flash'  # 'flash' or 'decide'
        ...
        try:
            ...
            if slides_data and isinstance(slides_data, list) and len(slides_data) > 0 and isinstance(slides_data[0], dict) and 'content' in slides_data[0]:
                ...
                for idx, slide in enumerate(slides_data):
                    if not isinstance(slide, dict):
                        continue
                    slide_title = str(slide.get('title') or f'Slide {idx + 1}').strip()
                    slide_content = str(slide.get('content') or '').strip()
                    slide_type = str(slide.get('type') or 'bullet').strip()
                ...
                if not slides_data:
                    ...
                    slides_data = (result or {}).get("slides") or _get_default_slides(topic)
                ...
                for idx, slide_topic in enumerate(slides_data):
                    topic_str = str(slide_topic or f"Slide {idx + 1}").strip()
                    toc_structure['chapters'].append({
                        'chapter_number': idx + 1,
                        'title': topic_str,
                        'sections': [{
            ...
            print(f"AI content generation notice: {e}, using template fallbacks")
            # Fallback: create basic content
            if slides_data and isinstance(slides_data, list) and len(slides_data) > 0 and isinstance(slides_data[0], dict):
                first_content = str(slides_data[0].get('content') or '') if slides_data else ''
                generated_content = {
                    "project_name": topic,
                    ...
                    "chapters": [
                        {
                            "chapter_number": idx,
                            "title": str(s.get('title') or f"Slide {idx}").strip().upper(),
                            "sections": [{
                                "number": f"{idx}.1",
                        ...
                        for idx, s in enumerate(slides_data[1:], 2)
                        if isinstance(s, dict)
                    ]
                }
            else:
                clean_slides = [str(s) for s in slides_data if s] if slides_data else _get_default_slides(topic)
                generated_content = _create_fallback_content(topic, clean_slides)
        ...
        try:
            ...
            if output_path.exists():
                import io
                # Read the generated presentation into an in-memory buffer
                return_data = io.BytesIO()
                with open(output_path, 'rb') as f:
                    return_data.write(f.read())
                return_data.seek(0)
                
                # Delete the temporary file from the disk to prevent disk space leaks
                try:
                    output_path.unlink()
                    print(f"   🗑️ Cleaned up temporary file: {output_filename}")
                except Exception as cleanup_err:
                    print(f"   ⚠️ Failed to delete temporary file {output_path}: {cleanup_err}")
